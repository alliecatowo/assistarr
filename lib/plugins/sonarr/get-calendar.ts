  ...
  return tool({
    ...
    execute: async ({ days, includePast }) => {
      try {
        ...
        const episodes = await client.get<SonarrCalendarEpisode[]>(
          "/calendar",
          {
        ...
        const calendarItems = episodes.map(formatCalendarItem);

        // Client sort since API returns chronologically by default but we map/process it
        calendarItems.sort(
          (a, b) =>
...
function formatCalendarItem(episode: SonarrCalendarEpisode) {
  const now = new Date();
