...
export const getCommandStatus = ({
  ...
  return tool({
    description:
      "Check the status of a Sonarr command. Use this after executing commands like manual import or scan to verify completion. Commands can be queued, started, completed, failed, or aborted.",
    inputSchema: z.object({
      commandId: z.number().describe("The command ID to check status for"),
    }),
    execute: async ({ commandId }) => {
      try {
        const command = await client.get<SonarrCommand>(
          `/command/${commandId}`
        );
...
function getStatusMessage(command: SonarrCommand): string {
